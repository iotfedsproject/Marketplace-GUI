...
import ReceiptBuyerItem from "./receipt-buyer-item.js"
...
import {
    auditReceiptBuyerVerify,auditReceiptBuyer,removeTokenFromExchange,makeExchangeableToken,searchAllExchangeTokens, searchMyExchangeTokens, searchOthersExchangeTokens,searchResourcesFromFederatedMarketPlace,fetchResourcesFromFederatedMarketplace,searchProductsFromFederatedMarketPlace,
...
class ReceiptBuyerAuditPage extends Component {
//----------------------------------------------------------------------------------------------------
  constructor(){
   super();
...
handleSearchReceipts = () =>{
 ...
 this.props.auditReceiptBuyer(jsonRequestData);
}//end
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
 ...
 const jsonRequestData = {
  ...
  this.props.auditReceiptBuyerVerify(jsonRequestData);
 }//end
//----------------------------------------------------------------------------------------------------
 componentDidMount() {
 }//end
...
//-----------------------------------------------------------------------------------------------------------
get renderBuyerReceiptItems(){
   if (this.props.federations.availableReceiptBuyer){

    let ReceiptsOfPurchasedProducts = <div><h1 style={{color: "#FFFFFF"}}><strong>Receipts of purchased products</strong></h1>  <h3 style={{color: "#FFFFFF"}}>No receipts of purchased products found</h3></div>;
    const { username } = this.props.userDetails;
    const filteredData = _.filter(this.props.federations.availableReceiptBuyer, { Buyer: username });
    var sizeOfReceiptBuyerItems = Object.keys(filteredData).length;

    {ReceiptsOfPurchasedProducts= _.map(filteredData, (member) =>
      <ReceiptBuyerItem ReceiptBuyerItem
    ...
    return (<div><h2 style={{color: "#FFFFFF",marginTop: '200px'}}><strong>Found {sizeOfReceiptBuyerItems} receipts of purchased products</strong></h2>{ReceiptsOfPurchasedProducts}</div>);
   }
//}
}//end
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
  render() {
    const { audit_receipt_buyer_error_message, auditReceiptBuyerMessage } = this.props.federations;

    return (
     <VisibilitySensor onChange={this.onChange}>
      <div className="container-fluid movie-app">
       <FieldError error={audit_receipt_buyer_error_message}/>
       <FieldSuccess message={auditReceiptBuyerMessage}/>


        {/*****************************************************/}
        <h1 style={{color: '#FFFFFF'}}><strong>Receipts of products purchased page</strong></h1>
        <div>
...
export default connect(mapStateToProps, {
    ...
    auditReceiptBuyer,
    auditReceiptBuyerVerify,
    exchangeTokens
})(withRouter(ReceiptBuyerAuditPage));
